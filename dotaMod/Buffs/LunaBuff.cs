    ...
    public class LunaBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ProjectileType<General.projectilestuffs.minions.LunaGlaiveSurfer>()] < 1)
            {
                ...
                Projectile.NewProjectile(player.Center.X + 200, player.Center.Y - 200, 0f, 0f, ProjectileType<LunaGlaiveSurfer>(), player.GetWeaponDamage(player.armor[k]), player.GetWeaponKnockback(player.armor[k], 2f), player.whoAmI);
